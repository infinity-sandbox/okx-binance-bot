...
class LeaderboardScraper:
    def __init__(self, db):
        self.db = db
        self.base_url = "https://okx-copy-trading.p.rapidapi.com"
        self.headers = {
            "X-RapidAPI-Key": rapid_api_key,
            "X-RapidAPI-Host": "okx-copy-trading.p.rapidapi.com"
        }
    ...
    async def bound_fetch(self, url, session, trader_id: str):
        try:
            ...
            # Handle the timeout error and return a custom response
            custom_response = "Timeout occurred"
            return {"response": custom_response, "trader_id": trader_id}
    ...
    def generate_positions_api_endpoints(
        self,
    ...
    def generate_historical_positions_api_endpoints(
        self,
        ignore_trader_ids: list = None,
        include_observed: bool = False
    ):
    ...
    def generate_user_statistics_api_endpoint_urls(
        self,
        querystring: dict,
        trader_ids: list = None,
        ignore_trader_ids: list = None
    ):
    ...
    def get_positions_from_api(self, ignore_trader_ids: list = None, include_observed: bool = False):
        ...
        for dict_i in results:
            ...
            # Fix received positions from API and hash it for comparing
            positions_fixed = []
            for position_dict in positions:
                ...
                # set every received position from API as not ignored and decide later if tis hould be ignored
                # position_dict_fixed["is_ignored"] = 0
                position_dict_fixed["trader_id"] = trader_id
    ...
    def get_historical_positions_from_api(
        ...
        for url in unique_urls:
            ...
            # 20 because it returns a max 20 results per single request
            ...
            for _ in range(total_pages_to_check + 1):
                ...
                if resp.json().get("message") == "OK":
                    ...
                    logger.warning(f"Failed to fetch traders from RapidAPI. Error: {resp.json().get('message')}")
                    return None  # Return None to indicate failure
    ...
    def get_init_traders(
        ...
        params_to_use = {"type": type_param}

        if (
            not validate_param(type_param, str, "type_param", ["pnl", "aum"])
            or not validate_param(max_traders_param, int, "max_traders_param")
            or not validate_param(lead_trader_assets_high_param, int, "lead_trader_assets_high_param")
            or not validate_param(aum_high, int, "aum_high")
            or not validate_param(
                time_as_lead_trader, int, "time_as_lead_trader", [7, 30, 90, 180]
            )
            or not validate_param(has_vacancies, bool, "has_vacancies")
            or not validate_param(aum_low, int, "aum_low")
            or not validate_param(lead_trader_assets_low, int, "lead_trader_assets_low")
            or not validate_param(win_ratio, float, "win_ratio")
        ):
            return None
        
        if lead_trader_assets_high_param:
            params_to_use["leadTraderAssetsHigh"] = str(lead_trader_assets_high_param)
        if aum_low:
            params_to_use["aumLow"] = str(aum_low)
        if lead_trader_assets_low:
        ...
        # Add valid params to params_to_use...
        # [Your existing logic to add parameters to params_to_use]
        ...
        while True:
            params_to_use["page"] = current_page

            # pprint(params_to_use, indent=4)
...
def update_trader_rois(retry_count: int = 20, include_observed: bool = True):
    ...
    all_trader_ids = []
    all_trader_ids += init_trader_ids
    if include_observed:
        observed_trader_ids = db.fetch_observed_trader_ids()
    else:
        observed_trader_ids = []

    all_trader_ids = list(set(observed_trader_ids + init_trader_ids))
    ...
    for retry_num in range(1, retry_count + 1):
        ...
        if retry_num == retry_count:  # last retry
            # This is the last retry, do not delay further
            break
...
def monitor_positions(retry_count: int = 20, delay: int = 10, include_observed: bool = False):
    ...
    while True:
        # logic for updating traders to follow and their ROIss
        current_day = datetime.today().strftime("%A")
        if top_traders_update_status["updated_on_day"] != current_day:
            is_right_time_to_update = helpers.is_valid_time_to_update_top_traders()
            if is_right_time_to_update or first_time_run:
                ...
                # if everything went fine, then update 'top_traders_update_status' to the current day name
                top_traders_update_status["updated_on_day"] = current_day
                first_time_run = False
                logger.success(f"Successfuly updated traders to follow for {current_day}.")
        ...
        for retry_num in range(1, retry_count + 1):
            ...
            if api_positions:
                ...
                if not os.path.exists("rapidapi_positions/"):
                    os.mkdir("rapidapi_positions/")
